...
# --- PAGE CONFIGURATION ---
st.set_page_config(
    page_title="NCDC Science Worksheet Architect",
    page_icon="🔬",
    layout="wide"
)

# --- CLEAN WORKSHEET TYPOGRAPHY (Science Green Theme) ---
st.markdown("""
    <style>
    * {
...
if "generated_sci_ws" not in st.session_state: st.session_state.generated_sci_ws = None
if "generated_sci_ws_key" not in st.session_state: st.session_state.generated_sci_ws_key = None
...
target_topic = st.sidebar.text_input("Worksheet Focus Topic / Theme", placeholder="e.g., The Human Eye, Crop Growing, or Sound Energy")

if st.sidebar.button("🗑️ Reset Application", use_container_width=True):
    st.session_state.generated_sci_ws = None
    st.session_state.generated_sci_ws_key = None
    st.rerun()

# --- MAIN INTERFACE ---
st.title("🔬 NCDC Daily Science Worksheet Architect")
st.subheader(f"Focused Topic Review — {target_class}")

st.markdown("""
<div class="blueprint-header">
    <strong>Enforced 10/10 Classroom Worksheet Blueprint:</strong><br>
    • <b>Section A: 10 Short Questions</b> Direct recall focusing on naming parts, stating functions, and identifying facts based on the target theme.<br>
    • <b>Section B: 10 Extended Tasks</b> Deep application questions featuring 3-part sub-questions (a, b, c), experiments, and diagram placeholders.
</div>
""", unsafe_allow_html=True)
...
if submit_btn:
    if not api_key_input or uploaded_file is None or not target_topic:
        ...
        with st.spinner(f"Drafting targeted worksheet for '{target_topic}'..."):
            ...
            system_instruction = (
                "You are an expert Primary Science curriculum designer creating classroom exercises based on the NCDC syllabus. "
                "CRITICAL INSTRUCTIONS:\n"
                "1. Output purely in clean Markdown text. NO LaTeX.\n"
                "2. The entire worksheet MUST focus primarily on the Target Focus Topic written by the teacher (e.g., if the topic is 'Immunization', all questions must revolve around vaccines, diseases, and health centres).\n"
                "3. Use familiar local Ugandan contexts (e.g., local crops, diseases, farming methods, and environment).\n"
                "4. You MUST separate the Worksheet from the Answer Key using exactly this delimiter on its own line: === SPLIT HERE ===\n\n"
                "WORKSHEET STRUCTURE:\n"
                "SECTION A: Short-Answer Foundation (Questions 1-10). Direct recall questions (naming, stating functions, basic identification). Provide a short blank line `______` for answers.\n"
                "SECTION B: Application & Diagrams (Questions 11-20). Multi-part questions broken down into (a), (b), (c). \n"
                "- DIAGRAMS: You MUST include at least 2 text placeholders for visual diagrams (e.g., '[Insert diagram of a maize seed here]' or '[Insert experiment showing light traveling in a straight line here]') and base the sub-questions on them."
            )
            ...
            Generate the targeted 20-question Science worksheet based on the focus topic and instructions, write the delimiter "=== SPLIT HERE ===", and then generate the Answer Key.
            """
            ...
            try:
                ...
                if "=== SPLIT HERE ===" in full_text:
                    ...
                    st.session_state.generated_sci_ws = parts[0].strip()
                    st.session_state.generated_sci_ws_key = parts[1].strip() if len(parts) > 1 else "Answer key missing."
                else:
                    st.session_state.generated_sci_ws = full_text
                    st.session_state.generated_sci_ws_key = "Answer key was not separated."
...
if st.session_state.generated_sci_ws:
    st.divider()
    st.markdown("### 📥 Download Your Classroom Materials")
    
    clean_title = f"{target_class}_{target_topic.replace(' ', '_')}_Worksheet"
    
    exam_pdf = create_pdf(st.session_state.generated_sci_ws, f"{target_class} - Science Worksheet ({target_topic})")
    exam_word = create_word_docx(st.session_state.generated_sci_ws, f"{target_class} - Science Worksheet ({target_topic})")
    key_word = create_word_docx(st.session_state.generated_sci_ws_key, f"{target_class} - Science Worksheet Answer Key")
    ...
    with st.expander("👁️ Preview Worksheet"): st.markdown(st.session_state.generated_sci_ws)
    with st.expander("👁️ Preview Answer Key"): st.markdown(st.session_state.generated_sci_ws_key)