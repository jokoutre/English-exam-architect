...
# --- PAGE CONFIGURATION ---
st.set_page_config(
    page_title="NCDC English Worksheet Architect",
    page_icon="📝",
    layout="wide"
)

# --- CLEAN WORKSHEET TYPOGRAPHY (Classic Crimson Theme) ---
st.markdown("""
    <style>
    * {
...
if "generated_eng_ws" not in st.session_state: st.session_state.generated_eng_ws = None
if "generated_eng_ws_key" not in st.session_state: st.session_state.generated_eng_ws_key = None
...
target_topic = st.sidebar.text_input("Worksheet Focus Topic / Theme", placeholder="e.g., Keeping Peace, Adjectives, or Letter Writing")

if st.sidebar.button("🗑️ Reset Application", use_container_width=True):
    st.session_state.generated_eng_ws = None
    st.session_state.generated_eng_ws_key = None
    st.rerun()

# --- MAIN INTERFACE ---
st.title("📝 NCDC Daily English Worksheet Architect")
st.subheader(f"Focused Topic Review — {target_class}")

st.markdown("""
<div class="blueprint-header">
    <strong>Enforced 10/10 Classroom Worksheet Blueprint:</strong><br>
    • <b>Section A: 10 Short Questions</b> Grammar focus (gap-filling, word forms, opposites, and sentence transformations) based on the target theme.<br>
    • <b>Section B: 10 Extended Tasks</b> A themed reading comprehension passage/poem (5 qs), jumbled sentences (4 qs), and a short guided composition (1 q).
</div>
""", unsafe_allow_html=True)

# --- PDF FILE UPLOAD ---
st.markdown("### 📄 Step 1: Upload NCDC English Curriculum Document")
...
if submit_btn:
    if not api_key_input or uploaded_file is None or not target_topic:
        ...
        with st.spinner(f"Drafting targeted worksheet for '{target_topic}'..."):
            ...
            system_instruction = (
                "You are an expert Primary English curriculum designer creating classroom exercises based on the NCDC syllabus. "
                "CRITICAL INSTRUCTIONS:\n"
                "1. Output purely in clean Markdown text. NO LaTeX.\n"
                "2. The entire worksheet MUST focus primarily on the Target Focus Topic written by the teacher (e.g., if the topic is 'Adjectives', all grammar and reading must revolve around adjectives and their usage).\n"
                "3. Use familiar local Ugandan names, places, and contexts.\n"
                "4. You MUST separate the Worksheet from the Answer Key using exactly this delimiter on its own line: === SPLIT HERE ===\n\n"
                "WORKSHEET STRUCTURE:\n"
                "SECTION A: Grammar & Vocabulary (Questions 1-10). Mix of gap-filling, word forms, opposites, and rewriting sentences. Provide a short blank line `______` for answers.\n"
                "SECTION B: Comprehension & Writing (Questions 11-20).\n"
                "- Q11-15: Write a short, engaging reading passage or poem (approx 100 words) related to the topic, followed by 5 comprehension questions.\n"
                "- Q16-19: Provide 4 jumbled sentences that, when rearranged, form a logical paragraph about the topic.\n"
                "- Q20: Provide a short guided composition task (e.g., 'Write a 5-sentence paragraph about...', or 'Write a short notice...')."
            )
            ...
            Generate the targeted 20-question English worksheet based on the focus topic and instructions, write the delimiter "=== SPLIT HERE ===", and then generate the Answer Key.
            """
            ...
            try:
                ...
                if "=== SPLIT HERE ===" in full_text:
                    ...
                    st.session_state.generated_eng_ws = parts[0].strip()
                    st.session_state.generated_eng_ws_key = parts[1].strip() if len(parts) > 1 else "Answer key missing."
                else:
                    st.session_state.generated_eng_ws = full_text
                    st.session_state.generated_eng_ws_key = "Answer key was not separated."
...
if st.session_state.generated_eng_ws:
    st.divider()
    st.markdown("### 📥 Download Your Classroom Materials")
    
    clean_title = f"{target_class}_{target_topic.replace(' ', '_')}_Worksheet"
    
    exam_pdf = create_pdf(st.session_state.generated_eng_ws, f"{target_class} - English Worksheet ({target_topic})")
    exam_word = create_word_docx(st.session_state.generated_eng_ws, f"{target_class} - English Worksheet ({target_topic})")
    key_word = create_word_docx(st.session_state.generated_eng_ws_key, f"{target_class} - English Worksheet Answer Key")
    ...
    with st.expander("👁️ Preview Worksheet"): st.markdown(st.session_state.generated_eng_ws)
    with st.expander("👁️ Preview Answer Key"): st.markdown(st.session_state.generated_eng_ws_key)